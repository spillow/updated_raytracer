...
AreaLight::AreaLight(point position, float side, int accuracy_factor,
                     float intensity)
{
    ...
    for (int i = 0; i < accuracy_factor; i++) // z move
    {
        for (int j = 0; j < accuracy_factor; j++) // x move
        {
            ...
            Light_Array[lightCount].set(
                point(cornerMiddle.px + ((float)j) * subSide + randX,
                      cornerMiddle.py,
...
{
    return Color(pRed * Color2.pRed, pGreen * Color2.pGreen,
                 pBlue * Color2.pBlue);
}
...
void Triangle::set(float p1x, float p1y, float p1z, float p2x, float p2y,
                   float p2z, float p3x, float p3y, float p3z)
{
    ...
    normal = ((Vertex[1] - Vertex[0]).crossProduct(Vertex[2] - Vertex[0]))
                 .normalize();
}

Box::Box(float width, float height, float depth, float transX, float transY,
         float transZ, float rotX, float rotY, float rotZ)
{
    ...
    pTriangles[0].set(-w, -h, d, w, -h, d, -w, h, d);
    pTriangles[1].set(-w, h, d, w, -h, d, w, h, d);
    pTriangles[2].set(-w, -h, -d, -w, h, -d, w, -h, -d);
    pTriangles[3].set(-w, h, -d, w, h, -d, w, -h, -d);
    pTriangles[4].set(-w, -h, -d, -w, -h, d, -w, h, -d);
    pTriangles[5].set(-w, h, -d, -w, -h, d, -w, h, d);
    pTriangles[6].set(w, -h, -d, w, h, -d, w, -h, d);
    pTriangles[7].set(w, h, -d, w, h, d, w, -h, d);
    pTriangles[8].set(-w, h, d, w, h, d, -w, h, -d);
    pTriangles[9].set(-w, h, -d, w, h, d, w, h, -d);
...
Cone::Cone(float radius, float height, int tess_factor, float transX,
           float transY, float transZ, float rotX, float rotY, float rotZ)
{
    ...
    for (int i = 0; i < tess_factor; i++)
        triangleStrip[i].set(radius * cos((float)i * angle_subtended),
                             radius * sin((float)i * angle_subtended), 0.0);
    ...
    {
        pTriangles[triangleNum].set(
            triangleStrip[i].px, triangleStrip[i].py, triangleStrip[i].pz,
            triangleStrip[i + 1].px, triangleStrip[i + 1].py,
            triangleStrip[i + 1].pz, apex.px, apex.py, apex.pz);
        triangleNum++;
        pTriangles[triangleNum].set(
            triangleStrip[i].px, triangleStrip[i].py, triangleStrip[i].pz,
            triangleStrip[i + 1].px, triangleStrip[i + 1].py,
            triangleStrip[i + 1].pz, circleCenter.px, circleCenter.py,
            circleCenter.pz);
        triangleNum++;
    }

    pTriangles[triangleNum].set(
        triangleStrip[tess_factor - 1].px, triangleStrip[tess_factor - 1].py,
        triangleStrip[tess_factor - 1].pz, triangleStrip[0].px,
        triangleStrip[0].py, triangleStrip[0].pz, apex.px, apex.py, apex.pz);
    triangleNum++;
    pTriangles[triangleNum].set(
        triangleStrip[tess_factor - 1].px, triangleStrip[tess_factor - 1].py,
        triangleStrip[tess_factor - 1].pz, triangleStrip[0].px,
        triangleStrip[0].py, triangleStrip[0].pz, circleCenter.px,
        circleCenter.py, circleCenter.pz);
...
void Octree::OctreeInit(BoundingBox **Box, Object *worldList,
                        int worldNumObjects)
{
    (*Box) = new BoundingBox;
...
bool Octree::boxSphereIntersection(BoundingBox Box, Object sphere)
{
    ...
    */
    ...
    if (sphere.getCenter().px < Box.boxMinExtent.px)
        dmin += (sphere.getCenter().px - Box.boxMinExtent.px) *
                (sphere.getCenter().px - Box.boxMinExtent.px);
    else if (sphere.getCenter().px > Box.boxMaxExtent.px)
        dmin += (sphere.getCenter().px - Box.boxMaxExtent.px) *
                (sphere.getCenter().px - Box.boxMaxExtent.px);

    // Y case
    if (sphere.getCenter().py < Box.boxMinExtent.py)
        dmin += (sphere.getCenter().py - Box.boxMinExtent.py) *
                (sphere.getCenter().py - Box.boxMinExtent.py);
    else if (sphere.getCenter().py > Box.boxMaxExtent.py)
        dmin += (sphere.getCenter().py - Box.boxMaxExtent.py) *
                (sphere.getCenter().py - Box.boxMaxExtent.py);

    // Z case
    if (sphere.getCenter().pz < Box.boxMinExtent.pz)
        dmin += (sphere.getCenter().pz - Box.boxMinExtent.pz) *
                (sphere.getCenter().pz - Box.boxMinExtent.pz);
    else if (sphere.getCenter().pz > Box.boxMaxExtent.pz)
        dmin += (sphere.getCenter().pz - Box.boxMaxExtent.pz) *
                (sphere.getCenter().pz - Box.boxMaxExtent.pz);
...
{
    // if we get below the threshold number of objects in a box, don't subdivide
    // any longer
    ...
    for (int z = 0; z < 2; z++)
    {
        for (int y = 0; y < 2; y++)
        {
            for (int x = 0; x < 2; x++)
            {
                displacement.set(diagonal.px * (float)x, diagonal.py * (float)y,
                                 diagonal.pz * (float)z);
                (*Box)->childBoxes[count++]->set((*Box)->boxMinExtent +
                                                     displacement,
                                                 boxCenter + displacement);
            }
        }
    }

    // now, see whether the children lie inside the child boxes
    // this requires a triangle-box intersection and sphere-box intersection
    // tests
    for (int i = 0; i < (*Box)->Storage.size(); i++)
    {
        for (int k = 0; k < 8; k++)
        {
            ...
            else
            {
                ...
                {
                    if (boxTriangleIntersection(
                            *((*Box)->childBoxes[k]),
                            (*Box)->Storage[i]->Triangle_Array[j]))
                    {
    ...
    // now, recursively go to each of the children and see if they need to be
    // subdivided
    for (int i = 0; i < 8; i++)
    {
...
bool Octree::rayBoxIntersection(BoundingBox Box, Ray ray)
{
    ...
    // X tests
    if (ray.getDirection().px < 0.0001 &&
        ray.getDirection().px > -0.0001) // ray parallel to yz-plane
    {
        if ((ray.getOrigin().px < Box.boxMinExtent.px) ||
            (ray.getOrigin().px > Box.boxMaxExtent.px))
            return false;
    }

    tIntersectMin =
        (Box.boxMinExtent.px - ray.getOrigin().px) / ray.getDirection().px;
    tIntersectMax =
        (Box.boxMaxExtent.px - ray.getOrigin().px) / ray.getDirection().px;
    ...
    // Y tests
    if (ray.getDirection().py < 0.0001 &&
        ray.getDirection().py > -0.0001) // ray parallel to yz-plane
    {
        if ((ray.getOrigin().py < Box.boxMinExtent.py) ||
            (ray.getOrigin().py > Box.boxMaxExtent.py))
            return false;
    }

    tIntersectMin =
        (Box.boxMinExtent.py - ray.getOrigin().py) / ray.getDirection().py;
    tIntersectMax =
        (Box.boxMaxExtent.py - ray.getOrigin().py) / ray.getDirection().py;
    ...
    // Z tests
    if (ray.getDirection().pz < 0.0001 &&
        ray.getDirection().pz > -0.0001) // ray parallel to yz-plane
    {
        if ((ray.getOrigin().pz < Box.boxMinExtent.pz) ||
            (ray.getOrigin().pz > Box.boxMaxExtent.pz))
            return false;
    }

    tIntersectMin =
        (Box.boxMinExtent.pz - ray.getOrigin().pz) / ray.getDirection().pz;
    tIntersectMax =
        (Box.boxMaxExtent.pz - ray.getOrigin().pz) / ray.getDirection().pz;
