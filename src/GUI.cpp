...
void callback_func(int caller)
{
    ...
    case 1:
        ...
        tracer.setResolution(X_Resolution->get_int_val(),
...
void MakeGUI()
{
    ...
    File_Textbox = glui->add_edittext_to_panel(Text_Panel, "SCENE:", 1, 0, 0,
                                               callback_func);
    ...
    Output_Textbox = glui->add_edittext_to_panel(Text_Panel, "OUTPUT:", 1, 0,
                                                 15, callback_func);
    ...
    Resolution_Panel =
        glui->add_panel_to_panel(Res_Options_Panel, "Resolution", true);
    X_Resolution = glui->add_spinner_to_panel(
        Resolution_Panel, "X:", GLUI_SPINNER_INT, 0, 2, callback_func);
    Y_Resolution = glui->add_spinner_to_panel(
        Resolution_Panel, "Y:", GLUI_SPINNER_INT, 0, 3, callback_func);
    ...
    reflections_spinner = glui->add_spinner_to_panel(
        Options_Panel, "Reflections", GLUI_SPINNER_INT, 0, 7, callback_func);
    blurry_spinner =
        glui->add_spinner_to_panel(Options_Panel, "Blurry Samples",
    ...
    check_output_file = glui->add_checkbox_to_panel(
        Options_Panel, "Output file", 0, 20, callback_func);
    ...
    Alias_Radio =
        glui->add_radiogroup_to_panel(Alias_Panel, 0, 14, callback_func);
